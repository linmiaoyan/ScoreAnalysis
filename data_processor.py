...
def _normalize_excluded_names(excluded_names: Optional[List[str]]) -> List[str]:
    if not excluded_names:
        return []
    out = []
    for n in excluded_names:
        nn = _normalize_student_name(n)
        if nn:
            out.append(nn)
    # 去重但保持顺序
    seen = set()
...
def _filter_df_excluded_students(df: pd.DataFrame, excluded_names: Optional[List[str]]) -> pd.DataFrame:
    """
    从 DataFrame 中剔除指定姓名的学生（用于不计入统计口径）。
    约定：若无“姓名”列则原样返回。
    """
    names = _normalize_excluded_names(excluded_names)
    if not names:
        return df
    ...
    if '姓名' not in df.columns:
        return df
    s = df['姓名'].astype(str).str.strip()
    return df[~s.isin(names)].copy()


def _filter_school_data_excluded(school_data: Dict[str, pd.DataFrame], excluded_names: Optional[List[str]]) -> Dict[str, pd.DataFrame]:
    """对我校数据结构（按学科拆表）统一剔除指定学生姓名。"""
    names = _normalize_excluded_names(excluded_names)
    if not names:
        return school_data
    out: Dict[str, pd.DataFrame] = {}
    for k, df in (school_data or {}).items():
        out[k] = _filter_df_excluded_students(df, names)
    return out
...
def analyze_school_total_score(
    school_data: Dict[str, pd.DataFrame],
    score_lines: List[float],
    ...
    # 可选：剔除指定学生（不计入统计）
    school_data = _filter_school_data_excluded(school_data, excluded_names)
...
def analyze_league_scores(
    ...
    # 可选：剔除指定学生（不计入统计）
    league_df = _filter_df_excluded_students(league_df, excluded_names)
    ...
    for line_name, subject_lines in (subject_lines_by_type or {}).items():
        ...
        for subject, score_line in subject_lines.items():
            ...
            try:
                ...
                continue
            # 选科场景：该学科为空白的学生，不计入该学科“参考人数/分母”
            col = pd.to_numeric(league_df[subject], errors='coerce')
            valid = col.notna()
            passed = valid & (col >= score_line)
            league_df_temp = league_df.copy()
            league_df_temp['_passed'] = passed.astype(int)
            league_df_temp['_total'] = valid.astype(int)
            by_school = league_df_temp.groupby('学校', as_index=False).agg(
                total=('_total', 'sum'),
                passed=('_passed', 'sum')
            )
            by_school['pass_rate'] = np.where(
                by_school['total'] > 0,
                (by_school['passed'] / by_school['total'] * 100).round(2),
                0.0
            )
...
def calculate_class_assessment(school_data: Dict[str, pd.DataFrame],
                              tekong_line: float,
                              yiduan_line: float,
                              excluded_names: Optional[List[str]] = None) -> Dict[str, List[Dict]]:
    """
    计算班级考核分
